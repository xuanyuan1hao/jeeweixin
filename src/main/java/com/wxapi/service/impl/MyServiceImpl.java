...
import com.core.util.QRCodeUtils;
...
public class MyServiceImpl implements MyService {
    ...
    private String processTextMsg(MsgRequest msgRequest, MpAccount mpAccount) {
        ...
        if (!StringUtils.isEmpty(content)) {//文本消息
            ...
            if (tmpContent.contains("领福利")){
                ...
                if(m.find()){
                    String referId=m.group(1);
                    System.out.println(m.group(1));
                    return updateDealReward(msgRequest,openId, referId,mpAccount);//处理红包发放
                }
            }
    ...
    //处理红包发放情况
    private String updateDealReward(MsgRequest msgRequest,String openId, String referId,MpAccount mpAccount) {
        MsgText msgText=new MsgText();
        String content="";
        //获取当前登录人信息
        AccountFans accountFans=accountFansService.getByOpenId(openId);
        if (null==accountFans){
            accountFans=syncAccountFans(openId,mpAccount,true);
        }
        if (accountFans.getUserReferId()!=0){
            content="已经领取了奖励，请生成海报转发推广获取更多奖励";
        }else{
            accountFansService.updateUserReferId(openId,Long.parseLong(referId));
            accountFans.setUserReferId(Long.parseLong(referId));
    ...
    private String processEventMsg(MsgRequest msgRequest, MpAccount mpAccount, String webRootPath, String webUrl) {
        ...
        if (MsgType.SUBSCRIBE.toString().equals(msgRequest.getEvent())) {//订阅消息
            ...
            if (null != fans) {
                ...
                accountFansService.updateUserAddMoney(fans, money, fans.getUserReferId(), mpAccount, 0);
            ...
            if (!StringUtils.isEmpty(key)) {
                ...
                if (key.startsWith("_fix_")) {
                    ...
                    if (null != accountFans) {
                        ...
                        if ((null == accountFans.getMediaId()) || ("".equals(accountFans.getMediaId()))) {
                            ...
                            createQRCode(accountFans, webRootPath, webUrl, mpAccount, userOpenId, headImg, false);
                ...
                } else if (key.indexOf("my_img_refer") >= 0) {
                    //获取图片推广地址，主要针对订阅号的二维码
                    //获取当前用户编号
                    //获取当前传送进来的URL地址
                    String userOpenId = msgRequest.getFromUserName();
                    AccountFans accountFans = accountFansService.getByOpenId(userOpenId);
                    if (null != accountFans) {
                        ...
                        String log = "生成我的海报大概需要5秒钟，请等待！";
                        log = getContent(MsgType.WaitCreateLog.toString(), log);
                        createQRCode(accountFans, webRootPath, webUrl, mpAccount, userOpenId, headImg, true);
                        MsgText msgResponseText = new MsgText();
                        msgResponseText.setContent(log);
                        return MsgXmlUtil.textToXml(WxMessageBuilder.getMsgResponseText(msgRequest, msgResponseText));
                    }
                }else if(key.indexOf("my_center_sub") >= 0){
                    String userOpenId = msgRequest.getFromUserName();
                    AccountFans accountFans = accountFansService.getByOpenId(userOpenId);
                    if (null != accountFans) {
                    MsgText msgResponseText = new MsgText();
                        String log = "点击以下链接进入会员中心:"+webUrl+"/wxapi/my_center_sub.html?openId="+accountFans.getOpenId()+"&id="+accountFans.getId();
                        msgResponseText.setContent(log);
                        return MsgXmlUtil.textToXml(WxMessageBuilder.getMsgResponseText(msgRequest, msgResponseText));
                    }
                }
    ...
    private void createQRCode(final AccountFans accountFans, final String webRootPath, final String webUrl,
                              final MpAccount mpAccount, final String userOpenId, final String headImg, final boolean isSubscribe) {
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    if (isSubscribe) {
                        //订阅号都是自己生成的二维码
                        String referUrl = webUrl + "/wxapi/my_referer.html?referId=" + accountFans.getId();
                        url = webRootPath+"/res/subscribe/" + accountFans.getOpenId() + "_refer.jpg";
                        if(!hasCreateRecommendPic(url)){
                            boolean createResult = QRCodeUtils.createQrEncode(webRootPath+"/res/subscribe/", accountFans.getOpenId() + "_refer.jpg", referUrl, 500, 500);
                            if (!createResult) {
                                throw new Exception("create QRCode error!");
                            }
                        }
                    } else {
                        //带参二维码
                        byte[] qrcode = WxApiClient.createQRCode(2592000, accountFans.getId().intValue(), mpAccount);
                        url = webRootPath + UploadUtil.byteToImg(webRootPath, qrcode);
                    }
    ...
    public AccountFans syncAccountFans(String openId, MpAccount mpAccount, boolean merge) {
        ...
        if (merge && null != fans) {
            ...
            if (tmpFans == null) {
                ...
                fans.setUserReferId(tmpFans.getUserReferId());
