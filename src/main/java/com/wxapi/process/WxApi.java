...
public class WxApi {
    ...
    //获取账号粉丝分组信息
    private static final String GET_FANS_GROUPS = "https://api.weixin.qq.com/cgi-bin/groups/get?access_token=%s";
    ...
    //获取粉丝分组接口
    public static String getFansGroupsUrl(String token) {
        return String.format(GET_FANS_GROUPS, token);
    }
    ...
    //获取上传图文消息接口
    public static String getUploadImageUrl(String token) {
        return String.format(UPLOAD_IMAGE, token);
    }
    ...
    public static JSONObject uploadMediaImage(String accessToken, String mediaUri) throws Exception {
        File file = new File(mediaUri);
        String uploadMediaUrl = String.format(UPLOAD_IMAGE, accessToken);
        URL uploadUrl = new URL(uploadMediaUrl);
